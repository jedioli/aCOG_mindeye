...
"""
 ...
 Modified for use with multitetris_adapt.py by: 
 ---------------------------------- (O)<< ----------------------------------------------
 ...
 Modified controls:
    ...
    -:          Remove a game
                (when used with Multi-Tetris_adapt)
    = (plus):   Add a game
                (when used with Multi-Tetris_adapt)
 ...
 STILL IN PRE-ALPHA STAGES
    ...
    rapid add/remove of games loses control, no window has focus
...
class game_controller(object):
    ...
    # ----------------- additions by OH ---------------------
        ...
        self.minus = False  # stores state for game removal
        self.plus = False  # stores state for game addition
        
        self.parent.bind("<space>", self.space_callback)
        
        self.parent.bind("-", self.minus_callback)  # TODO: replace with another method or API to remove control from user
        self.parent.bind("=", self.plus_callback)
    ...
    def random_rotate(self):
        ...
        after 5 sec from start, attempts to rotate every 0.1 sec
            rotates with approx. 3% of attempts
...
# ----------------- additions by OH ---------------------
    ...
    def minus_callback(self, event):
        """callback function for removing games
        integrates with multitetris_adapt.py
        
        final version will use external control
        """
        pass
        self.minus = True
        self.root.focus_force()
    
    def plus_callback(self, event):
        """callback function for adding games
        integrates with multitetris_adapt.py
